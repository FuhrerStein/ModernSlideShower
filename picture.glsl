...
void main() {
    ...
    tex_size = textureSize(texture_image, 0);
...
void main() {
    ...
    {
        pixel_color_hd = pixel_color_mixed(texture_image, uv0, pixel_size);
    ...
    {
        pixel_color *= .2;
        pixel_color += .2 * textureGrad(texture_image, uv0 + sub_pixel_distance * (dx + dy), dx, dy);
        pixel_color += .2 * textureGrad(texture_image, uv0 + sub_pixel_distance * (-dx + dy), dx, dy);
        pixel_color += .2 * textureGrad(texture_image, uv0 + sub_pixel_distance * (-dx - dy), dx, dy);
        pixel_color += .2 * textureGrad(texture_image, uv0 + sub_pixel_distance * (dx - dy), dx, dy);
    }
